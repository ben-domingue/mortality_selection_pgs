...
"no.death~raedyrs+bmi+height+smoke+cesd+diab+heart+alz+srh"->fm.baseline
...
tab<-list()
for (i in 3:length(dfL)) mod.fun(names(dfL)[i],dfL,fm=fm.baseline)->tab[[names(dfL)[i]]]
dev.off()
...
for (i in 3:length(dfL)) {
    tab.tmp<-list()
    for (ii in 1:100) {
        mod.fun(names(dfL)[i],dfL,fm=fm.baseline)->tab.tmp[[ii]]
    }
